...
noqapp.factory('httpServices', function($http, $q, $cookies, sessionService) 
{
	...
	loginData.getServicebyid=function(obj){
		var service=$http.get(studioURL+"studio_profile_detail/",{params:obj})
		return $q.all({'service_details':service})
	}

	loginData.getServiceType=function()
	{
		var service=$http.get(studioURL+"service_types/")
		return $q.all({'service_details':service})
	}	
	...
	loginData.splitBookings = function(bookings)
	{
		...
		{
			buking_date = new Date(bookings[i].appointment_date).getDate()
			if(bookings[i].booking_status == 'BOOKED' && bookings[i].status_code == 'B001' && 
			buking_date	>= today.getDate() )	
			{
